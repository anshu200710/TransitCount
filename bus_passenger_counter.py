...
BLOB_COMPACT_MAX = 25.0   # Max (perimeter² / area) compactness

# Ghost box extrapolation
GHOST_BOX_FRAMES = 45     # Synced with track_buffer
...
if __name__ == "__main__":
    ...
    p.add_argument("--debug",      action="store_true",   help="Enable detailed HSV debugging")
    p.add_argument("--head-detect",action="store_true",   help="Use CrowdHuman-trained model")
    p.add_argument("--visdrone",   action="store_true",   help="Use VisDrone-pretrained YOLOv8n")
    args = p.parse_args()
    BusCounter(
        args.source,
